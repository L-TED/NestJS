...
export class NotionService {
  ...
  async createKakaoDailyDb() {
    ...
    const body = {
      parent: { type: 'page_id', page_id: page },
      title: [{ type: 'text', text: { content: '카톡 신규 로그 DB' } }],
      properties: {
  ...
  async addData() {
    ...
    const database_id = '2ca111d189d781bbb102ff525920a841';
    const headers = {
      Authorization: `Bearer ${token}`,
      'Notion-Version': '2022-06-28',
      'Content-Type': 'application/json',
  ...
  async kakaoLog() {
    const filePath = path.join(__dirname, '../../groupA.txt');
    ...
    try {
      ...
      const token = this.config.get('TOKEN');
      const url = this.config.get('PAGEURL');
      const database_id = '2ca111d189d781f7bce0c524803cabf2';
